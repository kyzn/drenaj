...
def start(environment, n_child_processes=4):
    # run the worker
    os.system("celery multi start worker -l debug -f worker.log -c %s -A direnaj.client.celery_app.client_endpoint" % n_child_processes)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='direnaj client')
    ...
    if args.command == 'runserver':
        ...
        if keystore.no_access_tokens():
            ...
            if not keystore.no_access_tokens():
                start(DIRENAJ_VIS_ENVIRONMENT, args.n_children)
            else:
