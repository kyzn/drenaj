...
import util.MicromenderConfigurator;
import util.Util;
...
public class StopwordLabeler implements Labeler {
	...
	public static String excludeStopWords(String str) {
		return Util.excludeEach(str, findStopWords());
	}
	
	public static Set<String> findStopWords() {
		if (STOPWORDS == null) {
			STOPWORDS = new HashSet<String>();
			STOPWORDS.addAll(MicromenderConfigurator.getPropertyWithDelimeterAsStringList(MicromenderConfigurator.STOPWORDS, Util.DELIMITER_SPACE_STR));
		}
	...
	public List<Token> label(List<Token> tokens) {
		...
		for (Token token : tokens) {
			if (STOPWORDS.contains(Util.lowerCase(token.getValue()))) {
	...
	/*
	private static final String TWITTER_STOPWORDS = "fun nice love 1 2 3 4 5 6 7 8 9 0 check look brief live getting " 
