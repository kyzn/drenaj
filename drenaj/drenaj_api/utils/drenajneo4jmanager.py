...
def update_task_state_in_watchlist(user, since_tweet_id, page_not_found):
    ...
    graph.cypher.execute(
        "MATCH (u { id_str: {id_str} })<-[r:TIMELINE_TASK_STATE]-(task:TIMELINE_HARVESTER_TASK) " \
...
def init_user_to_graph_aux(campaign_node, user):
    ...
    if timeline_task_state_rel:
        ...
        tx.append(
            "MATCH (u:User),(t:TIMELINE_HARVESTER_TASK) WHERE u.id_str = {id_str} AND t.id = 1 "
    ...
    if friendfollower_task_state_rel:
        ...
        tx.append(
            "MATCH (u:User),(t:FRIENDFOLLOWER_HARVESTER_TASK) WHERE u.id_str = {id_str} AND t.id = 1 "
...
def upsert_campaign(campaign_id):
    campaign_node = graph.cypher.execute(
        "MATCH (c:Campaign) WHERE c.campaign_id = {campaign_id} RETURN c",
        {'campaign_id': campaign_id}).one
    print campaign_node
    if not campaign_node:
        campaign_node = graph.cypher.execute(
            "CREATE (c:Campaign {campaign_id: {campaign_id}}) RETURN c",
            {'campaign_id': campaign_id}).one
...
def get_users_attached_to_campaign(campaign_id, skip, limit):
    ...
    if campaign_node:
        ...
        user_nodes = graph.cypher.execute(
            "MATCH (c:Campaign { campaign_id: {campaign_id} })-[r:OBSERVES]->(user)<-[r2:FRIENDFOLLOWER_TASK_STATE|TIMELINE_TASK_STATE]-(x) RETURN user,collect(DISTINCT r2) as rlist SKIP {skip} LIMIT {limit}",
...
def add_to_watchlist(campaign_id, user_id_strs_to_follow, user_screen_names_to_follow):
    ...
    if user_screen_names_to_follow:
        users_to_be_added += prepare_users_to_be_added(user_screen_names_to_follow,
                                                       dtype='screen_name')
...
def create_batch_from_watchlist(app_object):
    ...
    graph.cypher.execute(
        "MATCH (u:User)<-[r:TIMELINE_TASK_STATE|FRIENDFOLLOWER_TASK_STATE|USER_INFO_HARVESTER_TASK_STATE]-(t) WHERE r.state = 1 AND r.unlock_time < {current_unix_time} SET r.state = 0, r.unlock_time = -1, r.updated_at = {current_unix_time}",
        {'current_unix_time': int(time.time())})
    ...
    res_array = []
    ff_task_states = graph.cypher.execute(
        "MATCH (u:User)<-[r:FRIENDFOLLOWER_TASK_STATE]-(t) WHERE (u.followers_count < 300000 AND u.friends_count < 300000) and r.state = 0 and (r.unlock_time = -1 "
        "OR r.unlock_time < {current_unix_time}) WITH r ORDER BY r.updated_at LIMIT {n_users}"
        " SET r.state = 1, r.unlock_time = {unix_time_plus_two_hours}, r.updated_at = {current_unix_time} RETURN DISTINCT r",
        {'n_users': n_users['friendFollowerTask'],
         'unix_time_plus_two_hours': int(time.time()) + (2 * 3600),
         'current_unix_time': int(time.time())})
    ...
    for ff_task_state in ff_task_states:
        ...
        res = app_object.send_task('crawl_friends_or_followers', [[job_definition]],
                                   queue='friendfollowers')
        res_array.append(res)
