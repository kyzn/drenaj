...
positional arguments:
  {runserver,dumpdb}
    runserver         run drenaj server
    dumpdb            dump drenaj database
...
# We need to make sure that 'drenaj' source directory is in the first path to look. Otherwise
# handlers cannot import utils.drnj_time because drenaj_api directory also has utils/ package. Since
# this app.py is in drenaj_api directory, python interpreter first looks whether utils are present or not
...
class Application(tornado.web.Application):
    def __init__(self, **overrides):
        ...
        # Creating a `handlers' variable using drenaj_routes_config.py
        from drenaj_api.config.drenaj_routes_config import routes_config as handlers

        settings_file = overrides.pop('settings_file', 'drenaj_api/config/settings.yaml')
...
def create_argument_parser():
    ...
    parser_runserver = subparsers.add_parser('runserver', description='Start Direnaj API Tornado application',
                                             ...
                                             help='run drenaj server',
                                             parents=[base_parser])
    parser_runserver.add_argument('-p', '--port', action='store', type=int,
                                  help='set drenaj api http port to listen')
    ...
    parser_runserver.add_argument('-s', '--settings-file', action='store', type=str,
                                  default='drenaj_api/config/settings.yaml',
